  ...
  <section class="mb-12">
    <h2 class="text-xl font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
      ...
      {#if flaggedImages.length > 0}
        <span class="px-2 py-0.5 rounded-full bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-sm">
          ...
          <div class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-hidden shadow-card">
            ...
            <div class="p-4">
              <div class="flex items-center justify-between mb-2">
                <a href="/peaks/{image.peak.slug}" class="text-sm font-medium text-sunrise hover:text-sunrise-coral">
                  {image.peak.name}
                </a>
              ...
              <div class="flex gap-2">
                <form method="POST" action="?/approveImage" use:enhance class="flex-1">
                  ...
                  <button type="submit" class="w-full px-3 py-2 rounded-lg bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 transition-colors">
                    Approve
                  </button>
                </form>
                <form method="POST" action="?/removeImage" use:enhance class="flex-1">
                  <input type="hidden" name="image_id" value={image.id} />
                  <button type="submit" class="w-full px-3 py-2 rounded-lg bg-red-600 text-white text-sm font-medium hover:bg-red-700 transition-colors">
                    Remove
                  </button>
                </form>
              </div>
  ...
  <section>
    <h2 class="text-xl font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
      ...
      {#if pendingFlags.length > 0}
        <span class="px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 text-sm">
          {pendingFlags.length}
        </span>
      {/if}
    </h2>
          ...
          <div class="flex items-center justify-between gap-4 p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
            ...
            <div class="flex gap-2 flex-shrink-0">
              ...
              <form method="POST" action="?/resolveFlag" use:enhance>
                ...
                <button type="submit" class="px-3 py-1.5 rounded-lg text-sm bg-red-600 text-white hover:bg-red-700">
                  Action
                </button>
              </form>
            </div>
